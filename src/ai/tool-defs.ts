...
const TOOL_DEFS_RAW: ToolDef[] = [
  {
    name: 'list_pages',
    description: `Shapion のすべてのページとデータベースの一覧を返す。
...
list_db_rows / create_db_row / update_db_row を呼ぶ前に必ずこれで列構成を確認すること。`,
    input_schema: {
      type: 'object',
      properties: {
        db_id: { type: 'string', description: 'DB の page id (shapion-pages の Id)' },
      },
      required: ['db_id'],
    },
  },
...
3. 必要なら body に行ページの markdown 本文を渡す（shapion-pages に保存される）
...
shapion-pages 上の対応する行ページ本文も同時に削除される（カスケード）。`,
    input_schema: {
      type: 'object',
      properties: {
        db_id:  { type: 'string' },
